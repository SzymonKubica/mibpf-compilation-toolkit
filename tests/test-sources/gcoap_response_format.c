...
int gcoap_response_format(bpf_coap_ctx_t *gcoap)
{
    ...
    if (pkt->payload_len >= str_len) {
        ...
        // It is very important that the programs modifying response packet buffer
        // return the correct length of the payload. This is because this return
        // value is then used by the server to determine which subsection of the
        // buffer was written to and needs to be sent back to the client.
        return pdu_len + str_len + start_len + end_len;
    }
    return -1;
}

// Returns the temperature in degrees Celsius with one decimal point. Represented
// as an integer (temperature * 10).
static int get_temperature()
{
    return -123;
}
