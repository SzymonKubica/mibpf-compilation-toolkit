...
#define DEBUG(...) do { if (ENABLE_DEBUG) { bpf_printf(__VA_ARGS__); } } while (0)
...
uint32_t sensor_processing(void *ctx)
{
    ...
    uint32_t temp = temperature_data.val[0];
    ...
    DEBUG("Pointer: %d\n", pointer);
    DEBUG("Pointer updated to: %d\n", (pointer + 1) % TEMP_STORAGE_SLOTS);
    DEBUG("Storing temperature in slot: %d\n", TEMP_DATA_START + pointer);
